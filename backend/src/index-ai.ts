...
app.get('/health', async (_req, res) => {
  try {
    ...
    const aiHealth = await AIServiceConfig.getInstance().healthCheck();
...
io.on('connection', (socket) => {
  ...
  socket.on('ai_request', async (data) => {
    try {
      console.log(`🤖 AI request from ${socket.id}:`, data.agentType);
      socket.emit('ai_response', {
        success: true,
        message: 'AI request received, processing...',
  ...
  socket.on('study_session_start', (data) => {
    console.log(`📚 Study session started: ${data.subject} by ${socket.id}`);
...
async function startServer() {
  try {
    console.log('🚀 Starting AARAMBH AI Backend Server with AI Services...');
    ...
    try {
      ...
      console.warn('⚠️  Anthropic initialization failed:', error);
    }
    ...
    server.listen(PORT, () => {
      ...
      console.log(`🎓 AI Tutor: http://localhost:${PORT}/api/ai/tutor/ask`);
      console.log(`📝 Content Creator: http://localhost:${PORT}/api/ai/content/create`);
      console.log(`📊 Assessment Generator: http://localhost:${PORT}/api/ai/assessment/create`);
    });
